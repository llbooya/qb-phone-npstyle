...
RegisterNUICallback('SendGroupChatMessage', function(data, cb)
    ...
    for k, room in pairs(PhoneData.ChatRooms) do
        if(room.id == Message.room_id) then
         ...
         TriggerServerEvent('qb-log:server:CreateLog', 'discordia', 'Oda No: '..data.roomID..'\'den yeni ileti', 'black', '**'..PhoneData.PlayerData.charinfo.firstname .. ' ' ..  PhoneData.PlayerData.charinfo.lastname..' : **'..data.message, false)
