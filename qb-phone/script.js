...
`
    ...
    resize(screenshot) {
        ...
        if (screenshot === true) {
            ...
            const width = Math.floor(window.innerHeight * 20 / 16);
            cameraRTT.setViewOffset(window.innerWidth, window.innerHeight, window.innerWidth / 6.5, 0, width, window.innerHeight);
        }
    ...
    requestScreenshot = (url, field , element) => new Promise((res) => {
        ...
        .then(text => {
           let textparse = JSON.parse(text);
            console.log(JSON.stringify(textparse));
                console.timeEnd("requestScreenshot");
                res(textparse.attachments[0].url);
            isAnimated = false;
        });
    })
